...
class KSLWorkspace {
    ...
    async initializeConversationUI() {
        ...
        // AI Refine toggle
        this.aiRefineEnabled = true; // Default true
        const aiToggle = document.getElementById('aiRefineToggle');
        if (aiToggle) {
            aiToggle.addEventListener('change', (e) => {
                this.aiRefineEnabled = e.target.checked;
            });
        }
    ...
    async generateSentence() {
        ...
        // Show loading state
        document.getElementById('sentenceText').innerHTML = '<span class="loading-pulse">AI가 문장을 다듬고 있습니다...</span>';
        document.getElementById('generateSentenceBtn').disabled = true;

        try {
            if (this.aiRefineEnabled) {
                // Call Gemini API
                ...
                if (data.success) {
                    ...
                    console.warn('AI Refinement failed, falling back to rule-based:', data.message);
                    sentence = this.addParticlesToWords(words);
                }
            } else {
