...
def api_refine():
    """Gemini를 이용한 자연어 정제 API"""
    try:
        data = request.get_json() or {}
        words = data.get('words', [])

        if not words:
            return jsonify({'success': False, 'message': 'words list is required.'}), 400

        # 단어가 하나만 있으면 그냥 반환 (또는 조사 붙이기)
        if len(words) == 1:
            return jsonify({'success': True, 'result': words[0]})

        if not model:
            # Fallback to local rule-based if API key is missing
            return jsonify({'success': False, 'message': 'Gemini API not configured', 'fallback': True})

        # Prompt construction
        word_list = ', '.join(words)
        prompt = f"""
        다음 단어들은 수어(Sign Language) 인식 결과입니다. 문맥을 고려하여 자연스러운 한국어 문장으로 연결해주세요.
        단어의 순서를 크게 바꾸지 말고, 적절한 조사와 어미를 붙여주세요.
        존댓말(해요체)을 기본으로 사용하세요.
        ...
        return jsonify({'success': True, 'result': refined_sentence})

    except Exception as e:
        print(f"Error in refine API: {e}")
